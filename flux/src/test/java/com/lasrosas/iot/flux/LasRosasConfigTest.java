...
@EnableIntegration
@ContextConfiguration(classes = {LasRosasIotConfig.class, SensorsConfig.class, IOTDatabaseConfig.class, UtilsConfig.class})
@DataJpaTest()
...
public class LasRosasConfigTest {
	...
	public void test() throws Exception {
		...
		message.setData("glop");
		message.setDataEncode("xxx");
		message.setDevEUI("123456");
		message.setDeviceName("Yop");
		message.setFCnt(1);
		message.setFPort(1);
		...
		log.info("Waiting for error");
		var error = errorChannel.receive(5000);
		if( error != null ) {
			throw new RuntimeException((Exception)error.getPayload());
		}
		log.info("Ok, no error");
		} catch(Exception e) {
			e.printStackTrace();
			throw e;
		}
